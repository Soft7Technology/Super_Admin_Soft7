...
const NAV = [
  { icon:"⊞",  label:"Dashboard",       bg:"#3b5bdb" },
  { icon:"🏢", label:"Manage Companies", bg:"#1971c2", badge:12 },
  { icon:"👥", label:"All User",         bg:"#0ca678" },
  { icon:"💳", label:"Subscription",     bg:"#6741d9" },
  { icon:"📋", label:"Audit Logs",       bg:"#862e9c" },
  { icon:"⚙️", label:"System",           bg:"#495057" },
  { icon:"👤", label:"Profile",          bg:"#1864ab" },
  { icon:"🎫", label:"Support Tickets",  bg:"#c92a2a" },
];

export default function Sidebar({ activeItem="Dashboard", onNavigate }: { activeItem?:string; onNavigate?:(l:string)=>void }) {
  ...
  return (
    <aside style={{ width:"250px", background:t.surface, borderRight:`1px solid ${t.border}`, display:"flex", flexDirection:"column", height:"100vh", position:"fixed", left:0, top:0, zIndex:100, transition:"background 0.3s,border-color 0.3s" }}>
      ...
      <nav style={{ flex:1, overflowY:"auto", padding:"12px 10px" }}>
        {NAV.map(item => <NavItem key={item.label} item={item} active={item.label===activeItem} onClick={()=>onNavigate?.(item.label)} t={t} isDark={isDark} />)}
      </nav>
...
function NavItem({ item, active, onClick, t, isDark }: any) {
  ...
  return (
    <div onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ display:"flex", alignItems:"center", gap:"12px", padding:"9px 12px", marginBottom:"2px", borderRadius:"9px", cursor:"pointer", userSelect:"none", transition:"all 0.15s",
      ...
      <div style={{ width:"30px", height:"30px", borderRadius:"8px", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"0.85rem", flexShrink:0, transition:"background 0.15s",
      ...
      {item.badge && <span style={{ background:"#1971c2", color:"#fff", fontSize:"0.62rem", padding:"2px 7px", borderRadius:"20px", fontWeight:700 }}>{item.badge}</span>}
      {active && <div style={{ width:"6px", height:"6px", borderRadius:"50%", background:t.accent, boxShadow:`0 0 6px ${t.accent}` }} />}
    </div>
  );
}
